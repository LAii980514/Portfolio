export const mainProjects = [
  {
    id: 1,
    title: "빛에 잠긴 미술관",
    genre: "액션 어드벤처",
    engine: "언리얼 엔진 5.7",
    mapSize: "36일",
    role: "12분",
    bullets: [
      "환경 내러티브로 동선을 읽게 하는 길찾기 설계",
      "기믹을 학습하고 응용하게 하는 퍼즐 구성",
      "전투, 잠입, 파쿠르, 수영을 배치한 다양한 플레이 경험"
    ],
    tags: ["파쿠르", "창작 레벨", "안도 다다오", "미술관"],
    alignImage: 'left',
    videoId: 'b3vxNhd6nbE',
    mainImage: '/project1_main_v2.jpg',
    docLink: 'https://drive.google.com/file/d/1jysxyHU7Q2F2xd7nmo0FlC9DyJctEt4V/view?usp=sharing', // 기획서 링크 업데이트
    coreExperience: '각 공간의 테마에 맞춰 서로 다른 플레이 경험을 배치해, 구간마다 색다른 재미로 레벨의 밀도를 높인다.',
    references: '언차티드 4, 스텔라 블레이드, 툼레이더',
    screenshots: [
      '/project1_screen1.jpg',
      '/project1_screen2.jpg',
      '/project1_screen3.jpg',
      '/project1_screen4.jpg',
      '/project1_screen5.jpg',
      '/project1_screen6.jpg',
      '/project1_screen7.jpg',
      '/project1_screen8.jpg'
    ]
  },
  {
    id: 2,
    title: "마법소녀의 기억 (폴리싱 중)",
    genre: "내러티브 액션 RPG",
    engine: "언리얼 엔진 5.5",
    mapSize: "37일",
    role: "8분",
    bullets: [
      "환경 기믹의 자연스러운 반복 학습과 테마별 응용 배치",
      "오브젝트 배치를 통한 직관적인 상황 암시 및 추리 요소 제공",
      "공간 테마와 동선 변화를 활용한 감정선 및 내러티브 연출"
    ],
    tags: ["어반판타지", "일본", "창작 레벨", "마법소녀"],
    alignImage: 'left',
    videoId: 'eioYulMp_5k',
    mainImage: '/project2_main.png',
    docLink: '#', // TODO: Add real Google Drive link
    coreExperience: '탁 트인 환경에서의 탐험과 거대 보스전의 내러티브 경험',
    references: '호라이즌 제로 던, 원신, 니어 오토마타'
  }
];

export const otherProjects = [
  {
    id: 101,
    title: '노바티아 평원',
    description: '자연물과 랜드마크를 활용해 동선을 유도하고, 탐험과 보상이 선순환되는 오픈월드 레벨 기획입니다.',
    tags: ['오픈월드', '탐험/채집', '시각적 유도'],
    year: '2024',
    genre: '액션 RPG',
    engine: '언리얼 엔진 5',
    mapSize: '45일',
    role: '12분',
    coreExperience: '명확한 랜드마크와 시각적 유도를 통한 자유도 높은 오픈월드 탐험 경험',
    docLink: 'https://drive.google.com/file/d/1DmuTaflTyD5ehoVilrYIn54dy7hasonz/view?usp=sharing',
    videoId: 'GLXku7q29JQ'
  },
  {
    id: 103,
    title: '침식당한 루테시아',
    description: '침식 스택 시스템과 보스 패턴을 파훼하기 위해 파티를 분할하여 역할을 분담하는 8인 레이드 보스전 기획입니다.',
    tags: ['8인 레이드', '협동 기믹', '파티 분할'],
    year: '2024',
    genre: '멀티플레이 레이드',
    engine: '언리얼 엔진 5',
    mapSize: '14일',
    role: '예상플레이 25분~30분',
    coreExperience: '상황에 따른 파티 분할과 오브젝트 투척 등 유기적인 협력을 요구하는 레이드 전투 경험',
    docLink: 'https://drive.google.com/file/d/1S6QoFr6Ce1Gl2yVJcnqcRNaZhmoS5Oeb/view?usp=sharing',
    videoId: 'sH5tL1whafg'
  },
  {
    id: 102,
    title: '죽음의 유적 동굴',
    description: '선형적인 구조 속에 숨겨진 장소를 배치하여 탐험의 재미를 더하고, 환경을 활용한 다채로운 전투를 유도하는 던전 기획입니다.',
    tags: ['던전', '숨겨진 장소', '점진적 난이도'],
    year: '2024',
    genre: '액션 RPG',
    engine: '언리얼 엔진 5',
    mapSize: '14일',
    role: '4분',
    coreExperience: '내부와 외부를 교차하며 점진적으로 상승하는 난도와 전투 패턴을 학습하는 경험',
    docLink: 'https://drive.google.com/file/d/1Rmr9RPlbrdRC5Rb8YEfyY7BLYIT4aJzx/view?usp=sharing',
    videoId: 'NqPXfOUyDkQ'
  }
];
